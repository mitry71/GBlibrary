        ...
        <div id="main">
            ...
            <div id="container">
                ...
                <div class="main-content">
                    ...
                    <div id="content">
                        <div id="_title">
                            <div class="artNum">글번호</div>
                            <div class="artTitle">글제목</div>
                            <div class="writeDate">작성일</div>
                        </div>
                        <c:choose>
                        	<c:when test="${empty articleList}">
                        		<p style="text-align:center; padding:20px;">등록된 글이 없습니다.</p>
                        	</c:when>
                        	<c:when test="${!empty articleList}">
		                        <c:forEach var="article" items="${articleList}" varStatus="articleNum">
			                        <div class="articles">
			                            <div class="artNum">${articleNum.count}</div>
			                            <div class="artTitle"><a href="${contextPath}/board/viewArticle.do?articleNo=${article.articleNo}">${article.title}</a></div>
			                            <div class="writeDate">${article.writeDate}</div>
			                        </div>
		                        </c:forEach>
                        	</c:when>
                        </c:choose>
					...
					<c:if test="${log_id == 'haejune'}">
						<div id="btnArea">
							<a href="${contextPath}/board/articleForm.do">공지사항 쓰기</a>
						</div>
					</c:if>
                </div>
                <!-- .main-content -->
